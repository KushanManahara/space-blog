...
export function TopicHeader({ topic }: { topic: Topic }) {
  return (
    <section className="relative">
      <div aria-hidden className="absolute inset-x-0 top-0 h-65 border-b border-line-1 bg-bg-3" />

      <div className="relative mx-auto max-w-page px-gutter pt-18.5">
        <Reveal className="flex flex-wrap items-center gap-[clamp(20px,3vw,32px)] rounded-xl border border-line-1 bg-bg-2 p-[clamp(24px,3vw,34px)] shadow-lg">
          <PostCover
            topic={topic.name}
            zoom={false}
